...
import com.pornhub.basic.util.RedisUtils;
...
public class RedisController implements RedisClient {
    ...
    public AjaxResult set(@RequestParam("key")String key, @RequestParam("value") String value){
        try {
            ...
            e.printStackTrace();
            return AjaxResult.getAjaxResult().setSuccess(false).setMessage("保存失败！"+e.getMessage());
        }
    ...
    public AjaxResult get(@RequestParam("key")String key){
        try {
            ...
            return AjaxResult.getAjaxResult().setSuccess(true).setMessage("成功").setRestObj(value);
        } catch (Exception e) {
            e.printStackTrace();
            return AjaxResult.getAjaxResult().setSuccess(false).setMessage("系统异常!"+e.getMessage());
        }
    }
